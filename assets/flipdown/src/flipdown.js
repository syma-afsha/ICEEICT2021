...
class FlipDown {
  ...
  _parseOptions(opt) {
    return {
      ...
      showEmptyRotors: opt.hasOwnProperty("showEmptyRotors")
        ? opt.showEmptyRotors
        : true,
    };
  }
  ...
  _setOptions() {
    ...
    this.opts.theme
      .split(",")
      .forEach((theme) =>
        this.element.classList.add(`flipdown__theme-${theme}`)
      );
  }
  ...
  _updateClockValues(init = false) {
    ...
    if (!init) {
      ...
      if (!this.opts.showEmptyRotors) {
        if (
          this.clockStrings.d == "00" &&
          this.clockStrings.h == "00" &&
          this.clockStrings.m == "00" &&
          this.clockStrings.s == "00"
        ) {
